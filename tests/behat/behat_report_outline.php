...
/**
 * Class behat_report_bbboverview custom Behat steps for report_bbboverview.
 */
class behat_report_bbboverview extends behat_base {
    /**
     ...
     *
     * @Given /^the log timestamp for "(?P<username>(?:[^"]|\\")*)" and "(?P<bbboverview_idnumber>(?:[^"]|\\")*)" is set to "(?P<date>(?:[^"]|\\")*)"$/
     * @param string $username
     * @param string $bbboverviewidnumber
     * @param string $date
     */
    public function the_log_timestamp_for_and_is_set_to($username, $bbboverviewidnumber, $date) {
        global $DB;
        ...
        // Find the log entry.
        $cmrec = $DB->get_record('course_modules', ['idnumber' => $bbboverviewidnumber], '*', MUST_EXIST);
